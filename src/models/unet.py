...
class DecoderBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # Апсемплинг через транспонированную свёртку
        ...
        # Свёрточный блок после конкатенации
        self.conv = ConvBlock(in_channels // 2 + skip_channels, out_channels)

    def forward(self, x: torch.Tensor, skip: torch.Tensor) -> torch.Tensor:
        x = self.up(x)

        # Выравнивание размеров если нужно (из-за нечётных размеров)
...
class ResNetEncoder(nn.Module):
    ...
    def __init__(self, pretrained: bool = True) -> None:
        super().__init__()
        # Загрузка pretrained ResNet-50
        weights = models.ResNet50_Weights.IMAGENET1K_V2 if pretrained else None
        resnet = models.resnet50(weights=weights)

        # Разбиение на блоки для извлечения признаков
        ...
        # Выходные каналы для skip connections
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        # Decoder blocks (от глубокого к поверхностному)
        ...
        # Финальный апсемплинг до исходного размера
